...
def default_project(name: str) -> dict:
    return {"name": name, "last_level": None, "level_history": [], "queue": []}
...
def load_state():
    loaded_agents = kv_get("agents", None)
    merged_agents = seed_agents()
    if loaded_agents:
        merged_agents.update(loaded_agents)  # garde les sous-agents et modifs déjà sauvegardés
    STATE["agents"] = merged_agents
    save_agents()  # ré-enregistre la version fusionnée pour les prochains démarrages
...
def call_grounded(prompt: str) -> str:
    """Appel avec la recherche Google activée (grounding). Si le quota
    Gemini est épuisé, bascule sur Groq — mais sans recherche web réelle
    dans ce cas (Groq ne propose pas cette fonctionnalité), en le précisant
    clairement dans la réponse."""
    from google.genai import types
    ...
    try:
        ...
        if _is_quota_error(e):
            fallback = call_groq(
                ...
                "(tu n'as pas accès à une recherche web en direct dans ce mode).",
                prompt,
            )
            return "⚠️ Recherche web indisponible (quota Gemini épuisé) — réponse basée sur les connaissances générales du modèle de secours :\n\n" + fallback
        raise
...
@app.get("/")
def root():
    """Page d'accueil simple — confirme que le serveur tourne et donne la
    version déployée, pratique pour vérifier qu'un fichier n'a pas été
    copié-collé de façon incomplète."""
    return {
        "app": "ForgeGameIA Backend",
...
def debug_routes():
    """Liste toutes les routes actuellement actives sur ce déploiement.
    Utile pour vérifier qu'une route attendue (ex: /studio-learn) est bien
    présente après une mise à jour du fichier main.py."""
    return sorted(
...
# ---------------------------------------------------------------------
# File d'attente — traitement automatique de nuit (via GitHub Actions
# programmé), pour lisser l'usage du quota gratuit sans surveillance.
# Les agents ne modifient et ne poussent JAMAIS de code automatiquement :
# ils déposent leurs réponses/rapports, à lire et appliquer toi-même.
# ---------------------------------------------------------------------
...
def queue_secret_ok(provided: str | None) -> bool:
    expected = os.environ.get("QUEUE_SECRET")
    return bool(expected) and provided == expected


@app.post("/queue")
def add_to_queue(item: QueueItem):
    """Dépose un prompt à traiter automatiquement lors du prochain passage
    nocturne, plutôt que tout de suite."""
    import uuid
    import datetime
    project = active_project()
    ...
    project["queue"].append(entry)
    save_projects()
    return {"status": "ok", "queued_id": entry["id"]}
...
def process_queue(x_queue_secret: str | None = Header(default=None)):
    """Traite les prompts en attente (max 5 par passage, pour rester dans
    le quota gratuit). Déclenché chaque nuit par un workflow GitHub Actions
    programmé — jamais de modification de code automatique, uniquement des
    réponses/rapports déposés pour relecture."""
    if not queue_secret_ok(x_queue_secret):
        raise HTTPException(401, "QUEUE_SECRET invalide ou manquant.")

    project = active_project()
    queue = project.get("queue", [])
    pending = [e for e in queue if e["status"] == "en_attente"][:5]
    processed_count = 0

    for entry in pending:
        try:
            agent_id = route_with_manager(entry["message"], [])
            agent = STATE["agents"][agent_id]
            raw = call_agent(agent["system_prompt"], entry["message"], [])
            if agent["output_type"] == "level_json":
                try:
                    level = extract_json(raw)
                    project["last_level"] = level
                    entry["result"] = f"[{agent['name']}] Niveau \"{level.get('name', '?')}\" généré."
                except json.JSONDecodeError:
                    entry["result"] = f"[{agent['name']}] Réponse JSON invalide."
            else:
                entry["result"] = f"[{agent['name']}] {raw}"
            entry["status"] = "traité"
        except Exception as e:
            entry["result"] = f"Erreur: {e}"
            entry["status"] = "échec"
        processed_count += 1

    save_projects()
    return {"processed": processed_count, "remaining": len([e for e in queue if e["status"] == "en_attente"])}
